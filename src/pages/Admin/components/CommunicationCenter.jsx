...
export function CommunicationCenter({ initialChatName, initialTab = 'messages' }) {
  ...
  // رسائل تجريبية حسب الـ chat المختار
  const messages = selectedChat
    ? [
        {
          id: 1,
  ...
  const handleSendMessage = () => {
    if (message.trim()) {
      console.log('Sending message:', message, 'to', selectedChat?.name);
      setMessage('');
    }
  };

  // لو جاي initialTab من برة
  useEffect(() => {
    if (initialTab) {
  ...
  // لو جاي اسم الشخص من برة افتحه مباشرة
  useEffect(() => {
    if (initialChatName) {
