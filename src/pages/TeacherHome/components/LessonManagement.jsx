...
function LessonManagement({ onNavigate }) { // ✅ لو بتستعمليه بالـ TeacherHome خليه
  const navigate = useNavigate();
  const [showModal, setShowModal] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  ...
  const handleCreateLesson = () => {
    ...
    // ✅ مباشرة بعد الإنشاء، افتحي واجهة lesson videos
            ...
            {filteredLessons.length > 0 ? (
              filteredLessons.map((lesson) => (
                <div
                  ...
                  onClick={() => handleOpenLesson(lesson.id)} // ✅ فتح صفحة lesson videos
                  style={{ cursor: 'pointer' }}               // إحساس إنها clickable
                >
                  <div className="lesson-card-header">
                    <div className="lesson-header-section">
                      <div className="lesson-title-section">
